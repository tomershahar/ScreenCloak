...
- **PyInstaller** — same as macOS, produces `dist/SafeStream.exe`
...
| File | Purpose |
|------|---------|
| `SafeStream-Windows.spec` | PyInstaller config for Windows (no macOS entitlements/LSUIElement) |
| `scripts/build_windows.bat` | One-command build: PyInstaller + Inno Setup |
| `scripts/safestream.iss` | Inno Setup script — installer UI, shortcuts, uninstaller |

### Build Environment

Windows builds must run on a Windows machine (PyInstaller does not support cross-compilation). The build produces `dist/SafeStream-1.0.0-Setup.exe`.
